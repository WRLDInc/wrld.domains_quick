...
Cloudflare Pages auto-deploys from GitHub on push. For manual deploys: `npm run build && npm run deploy`. Security and cache headers are set in `public/_headers` (fonts are listed by exact filename so a missing font never caches a 404). `www.wrld.domains` currently returns 522 at the edge: the `_redirects` rule only works once the `www` hostname is attached to the Pages project.
